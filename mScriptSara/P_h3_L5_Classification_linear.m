...
%%
h3_CLmnr_l5  = cell2table(cell(0,7), 'VariableNames',...
    ...
    for t = 1:yc-1
        ...
        for k = 1:xc-1
            ...
            for i = 1:size(m,1)
                ...
                %[mnr,dev,stats]
                mnr = mnrfit(X(trainInd,input),Y(trainInd,t),'Model','ordinal');
                
                % Test the model
                pihat = mnrval(mnr,X(testInd,input),'Model','ordinal');
                [~,idx]=max(pihat,[],2);
                
                % Record every itration information
                PathCode = pathlist{p};
                StudentNum = size(pathRecords,1);
                TrainingNum = size(X(trainInd,input),1);
                PredictorsNum = xc-1;
...
%%
plot(h3_CLmnr_l5.PredictorsNum,h3_CLmnr_l5.Accuracy,'mx','MarkerSize',7)
...
h3_CLmnr_l5_mean=grpstats(h3_CLmnr_l5,{'PathCode','StudentNum','TrainingNum','PredictorsNum'}, {'mean'} ,'DataVars',{'Accuracy', 'Error'});
h3_CLmnr_l5_mean(:,5)=[];
%%
plot(h3_CLmnr_l5_mean.PredictorsNum,h3_CLmnr_l5_mean.mean_Accuracy,'mx','MarkerSize',7)
title(sprintf('P(L5 module|L4 modules)\n\n Multinomial Logistic Regression \n'));
xlabel('Number Of Predictors');
ylabel('Prediction Accuracy (mean)');
print('figures/p_l5_mnr_Accuracy_vs_PredictorsNum_mean','-dpng');
%%
plot(h3_CLmnr_l5_mean.TrainingNum,h3_CLmnr_l5_mean.mean_Accuracy,'mx','MarkerSize',7)
title(sprintf('P(L5 module|L4 modules)\n\n Multinomial Logistic Regression \n'));
xlabel('Number Of Trining Observation');
ylabel('Prediction Accuracy (mean)');
