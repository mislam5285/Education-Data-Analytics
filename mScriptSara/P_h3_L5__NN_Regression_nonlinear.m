...
%%
h3_NN_l5  = cell2table(cell(0,13), 'VariableNames',...
         ...
         for k = 1:xc-1
             ...
             for i = 1:size(m,1)
                 ...
                 for h = 1:xc-1 % loop through range of different hidden layer sizes (no greater than the number of predictors)
                     ...
                     rng('default');
                     % train the model
                     [net,tr] = train(net,X(trainInd,input)',Y(trainInd,t)');
                     
                     % Test the model using the validate set
                     VY = net(X(ValidInd,input)');
                 ...
                 % train the model
                 [net,tr,trainY ,e] = train(net,X(trainInd,input)',Y(trainInd,t)');
                 
                 % test the model using the testing set
                 TestY = net(X(testInd,input)');
                 ...
                 temp_ = cell2table({ PathCode, StudentNum, TrainingNum, PredictorsNum,Predictors, ...
                     Target, bestH ,R2, MSE, RMSE, PredR2, PredMSE, PredRMSE },'VariableNames', {'PathCode', ...
                     'StudentNum', 'TrainingNum', 'PredictorsNum','Predictors', 'Target',...
                     'H','R2', 'MSE', 'RMSE','PredR2','PredMSE', 'PredRMSE'});
                 
                 h3_NN_l5 = [h3_NN_l5; temp_];
             end
         end
     end
...
filename = 'h3_NN_l5.csv';
writetable(h3_NN_l5,filename);
